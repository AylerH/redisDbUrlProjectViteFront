...
import { matchCompanyByBlankName } from '../api/redisApi';

const FuzzyMatchBlankName: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await matchCompanyByBlankName(dbName, query, minSimilarity, limit);
      setResult(response);
    } catch (err) {
  ...
  return (
    <div className="fuzzy-match-container">
      <h2>空格分隔条件名称模糊匹配</h2>
      <p className="description">输入多个空格分隔的关键词，系统将匹配同时包含这些关键词的公司名称</p>
      ...
      <div className="search-form">
        <div className="form-group">
          ...
          <input
            ...
            placeholder="输入空格分隔的关键词 (例如: 科技 有限 公司)"
          />
        </div>
...
export default FuzzyMatchBlankName; 